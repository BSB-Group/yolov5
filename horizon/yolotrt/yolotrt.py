...
from .postprocessing import cxcywh_to_xyxy, xyxy_to_xyxyn, scale_boxes, nms
...
class YoloTRT:
    ...
    def __init__(self, trt_engine_path: str,
        ...
        self.cls_map = load_labels(cls_map_fpath) if os.path.exists(cls_map_fpath) else None
        ...
        print("  * Max batch size - {}\n".format(self.trt_engine.max_batch_size))
    ...
    def __call__(self,
        ...
        img : np.ndarray
            The input image.
        half : bool, optional
            If True, the image is transformed to float16 for faster inference. Otherwise, float32.
        conf : float, optional
    ...
    def predict(self, img, conf_thresh, iou_thresh, half=True):
    ...
    def detect(self,
        ...
        - iou_thresh: float, optional
            Minimum IOU to be counted as a duplicate detection.
        - half: bool, optional
            Se to True when using float16 model.
        """
        ...
        img = self.preprocess(img, half)
        output = self.forward(img)
        bboxes, scores, classes = self.postprocess(
            output,
            orig_shape,
            conf_thresh=conf_thresh,
            iou_thresh=iou_thresh,
    ...
    def forward(self, img: np.ndarray) -> np.ndarray:
        ...
        # Reshape outputs
        detection_outs = [np.reshape(det, out.shape[1:])
                          for det, out in zip(detection_outs, self.outputs)]
    ...
    def preprocess(self, img: Image, half: bool = True) -> np.ndarray:
        """
        ...
        img = letterbox(img, target_shape=(self.img_size,) * 2)[0]
    ...
    def postprocess(self,
        ...
        orig_shape : tuple (int, int) | None
            Original image shape (height, width) or None to skip scaling.
        conf_thresh : float
